...
describe("Testing deployment of all pancakeSwap Exchange contracts", function () {
  ...
  it("Should approve tokens, then create a liquidity pool", async function () {
    const [owner, addr1, addr2] = await ethers.getSigners();
    ...
    console.log("init code hash:", init_code);

    //console.log(deadline, val1, val2, val3);
    await routerContract
      .connect(addr1)
      .addLiquidityETH(tokenERC20_address, val1, val2, val3, addr1.address, deadline, {
  ...
  it("should trade the pair with an other account", async function(){
    const [owner, addr1, addr2] = await ethers.getSigners();
    const WBNBContract = await ethers.getContractAt("WBNB", wbnb_address);
    const tokenERC20Contract = await ethers.getContractAt("MockERC20", tokenERC20_address);
    const factoryV2Contract = await ethers.getContractAt("PancakeFactory", factoryV2_address);
    ...
    await routerContract.connect(addr2).swapExactETHForTokens(ethers.utils.parseEther("0.1"), [wbnb_address,tokenERC20_address], addr2.address,
    deadline,{value: ethers.utils.parseEther("0.1"),});
 
    expect (await tokenERC20Contract.balanceOf(addr2.address)).to.be.above(0);
    const balance = await tokenERC20Contract.balanceOf(addr2.address);
    expect(balance, 'token balance of addr2 should be positive after the swap').to.be.above(0);

    //give approval to swap token back to ETh
    const approveVal = ethers.utils.parseEther("10000");
    await tokenERC20Contract.connect(addr2).approve(router_address, approveVal);
    
    await routerContract.connect(addr2).swapTokensForExactETH(ethers.utils.parseEther("0.08"), ethers.utils.parseEther("0.90702"), [tokenERC20_address,wbnb_address], addr2.address,
    deadline);
    const balance2 = await tokenERC20Contract.balanceOf(addr2.address);
    expect(balance2, 'token balance of addr2 should be diminished after trading back to eth').to.be.below(balance);
  })
});
